...
class CasManager
{
     ...
     * @var array
     */
    protected $providerMappers = array();
    ...
    public function getTokenClassNamesByProviderName($providerName)
    {
        ...
        return array(self::DEFAULT_CAS_TOKEN);
    }
    ...
    private function resolveResoursceToRealpath($resource, &$fileExists = null)
    {
        ...
        if ($isOnPackage) {
            ...
            $filename = \TYPO3\Flow\Utility\Files::concatenatePaths(array($package->getResourcesPath(), $uriParts['path']));
        } else {
            // will not work
            return null;
            $filename = \TYPO3\Flow\Utility\Files::concatenatePaths(array(FLOW_PATH_DATA, 'Persistent/Resources' . $uriParts['path']));
        }
